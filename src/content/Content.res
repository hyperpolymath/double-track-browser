...
let simulateScroll = (targetDepth: float, onComplete: unit => unit) => {
  ...
  let step = () => {
    ...
    let jitter = Js.Math.random() *. 80.0 +. 20.0
  ...
  let baseInterval = 100 + Belt.Float.toInt(Js.Math.random() *. 200.0)
...
let simulateMouseMovement = (onComplete: unit => unit) => {
  ...
  let numMoves = Belt.Float.toInt(Js.Math.random() *. 5.0) + 2
  let moveIndex = ref(0)

  let rec doMove = () => {
    if moveIndex.contents >= numMoves || Js.Array2.length(elements) == 0 {
      onComplete()
    } else {
      let idx = Belt.Float.toInt(Js.Math.random() *. Belt.Int.toFloat(Js.Array2.length(elements)))
      switch elements[idx] {
      | Some(el) =>
        dispatchMouseEvent(el, "mouseover")
        dispatchMouseEvent(el, "mousemove")
        // Pause like a human reading, then move on
        let delay = 300 + Belt.Float.toInt(Js.Math.random() *. 1200.0)
        let _ = setTimeout(() => {
...
let simulateClick = (onComplete: unit => unit) => {
  ...
  if Js.Array2.length(internalLinks) > 0 {
    let idx = Belt.Float.toInt(Js.Math.random() *. Belt.Int.toFloat(Js.Array2.length(internalLinks)))
    ...
    | Some(link) =>
      ...
      let _ = setTimeout(() => {
        ...
        onComplete()
      }, 500 + Belt.Float.toInt(Js.Math.random() *. 800.0))
    | None => onComplete()
    }
  } else {
    onComplete()
...
let runSimulation = (params: Types.simulationParams, onComplete: unit => unit) => {
  Js.Console.log("DoubleTrack: Starting page behavior simulation")
  ...
  simulateScroll(params.scrollDepth, () => {
    ...
    simulateMouseMovement(() => {
      ...
      if params.clickLinks && Js.Math.random() > 0.5 {
        simulateClick(() => {
...
let _ = Chrome.Runtime.onMessageAddListener((messageJson, _sender, sendResponse) => {
  ...
  | "SIMULATE_ACTIVITY" =>
    ...
    runSimulation(params, () => {
      Js.Console.log("DoubleTrack: Simulation complete, signaling background")
      let _ = Chrome.Runtime.sendMessage(
        Obj.magic({"type": "ACTIVITY_COMPLETE", "payload": Js.Nullable.null}),
      )
    })
